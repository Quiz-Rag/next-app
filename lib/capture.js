...
export const DUMPCAP = pickBin('dumpcap', 'DUMPCAP', [
  '/opt/homebrew/bin/dumpcap',
  '/usr/local/bin/dumpcap',
  '/Applications/Wireshark.app/Contents/MacOS/dumpcap',
  '/usr/bin/dumpcap',
]);
export const TSHARK = pickBin('tshark', 'TSHARK', [
...
export function startCapture(opts = {}) {
  ...
  console.log('[capture] cmd:', [DUMPCAP, ...args].join(' '));
  ...
  return { pcap, seconds, iface, cmd: [DUMPCAP, ...args].join(' ') };
}
...
function runTS(args) {
  return new Promise((resolve, reject) => {
    execFile(TSHARK, args, { maxBuffer: 20 * 1024 * 1024, encoding: 'utf8' },
      (err, stdout, stderr) => {
